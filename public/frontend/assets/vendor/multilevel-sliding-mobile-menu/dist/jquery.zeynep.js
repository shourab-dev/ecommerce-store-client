 ...
 */ (function ($) {
    ...
    Plugin.prototype.init = function () {
        ...
        submenuTriggers.each(function () {
            ...
            _this.on("click", function (event) {
                var scrollTop =
                    submenuEl.parents(".submenu:first").scrollTop() || 0;
            ...
            submenuEl
                .find('[data-submenu-close="' + subMenuId + '"]')
                .on("click", function (event) {
                    submenuEl.parents(".submenu:first").css("overflow-y", "");
                    submenuEl.css(
                        "transform",
                        "translateX(" + options.width + "px)"
                    );
    ...
    Plugin.prototype.open = function () {
        ...
        this.options.disableTransition &&
            this.element.add(html).addClass("no-transition");
    ...
    Plugin.prototype.close = function (disableEvent) {
        ...
        this.options.disableTransition &&
            this.element.add(html).removeClass("no-transition");
    ...
    var eventController = function (type) {
        if (!this.options[type] || typeof this.options[type] !== "function")
            return;
        switch (type) {
            case "onLoading":
